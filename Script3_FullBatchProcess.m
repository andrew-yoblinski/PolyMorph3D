...
for k = 1:numel(imgFiles)
    ...
    MeanFI_Glia = mean(imgGlia, 'all');
    ImageDataTable = table(MeanFI_Glia, ...
    'VariableNames', {'MeanFI_Glia'});
    ...
    fprintf('Normalizing glia channel \n');
    ...
    if SecondGliaChanYN == 1
        ...
        MeanFI_Glia_Ch2 = mean(imgGlia2, 'all');
        
        ImageDataTable.MeanFI_Glia_Ch2 = MeanFI_Glia_Ch2(:);
        
        globalMin = double(min(imgGlia2(:)));
        globalMax = double(max(imgGlia2(:)));
        globalRange = globalMax - globalMin;
        imgGliaNorm2 = zeros(size(imgGlia2), 'single');
        fprintf('Normalizing second glia channel with global scaling...\n');
    ...
    % Save max project of original primary glia channel
    fprintf('Saving original glia channel \n')
    img2D = max(imgGlia, [], 3);
    figName = [name,'_step0_Glia_OrigMAX'];
    fig = figure('Visible', 'off', 'Name', figName);
    imagesc(img2D);
    colormap gray;
    daspect([1 1 1]);
    ...
    % Save max project of original second glia channel
    if SecondGliaChanYN == 1
        fprintf('Saving original second glia channel \n')
    ...
    %% THRESHOLD PRIMARY GLIA CHANNEL
    % this section recapitulates the thresholding steps taken in
    % ARY_ThresholdGUI_App from Script1
    fprintf('Thresholding glia channel \n');

    switch ThreshMethod
        case 'Otsu'
            level = graythresh(imgGliaNorm(:));
        case 'Multi-Otsu'
            level = multithresh(imgGliaNorm(:));
        case 'Median'
            level = median(imgGliaNorm(:));
        case 'Mean'
            level = mean(imgGliaNorm(:));
    end

    if ThreshMode == 1
        BinaryImg = zeros(sizeXY(1), sizeXY(2), sizeZ);
        for i = 1:sizeZ
            BinaryImg(:, :, i) = imbinarize(imgGliaNorm(:, :, i), level * ThreshSet);
        end
    else
        BinaryImg = imbinarize(imgGliaNorm, level * ThreshSet);
    end

                    
    if FillGaps == 1
        radiusXY = FillGapsRadius;
        radiusZ = max(1, round(radiusXY * pxXY / pxZ));
        [x, y, z] = meshgrid(-radiusXY:radiusXY, -radiusXY:radiusXY, -radiusZ:radiusZ);
        se = (x.^2 + y.^2) / radiusXY^2 + z.^2 / radiusZ^2 <= 1;
    ...
    keepIdx = find(volumes >= noise.Min & volumes <= noise.Max);
    L = labelmatrix(CC);
    NoiseIm = ismember(L, keepIdx);

    % Threshold secondary glia channnel if applicable
    if SecondGliaChanYN == 1
        fprintf('Thresholding second glia channel \n');
    
        switch ThreshMethod2
            case 'Otsu'
                level = graythresh(imgGliaNorm2(:));
            case 'Multi-Otsu'
                level = multithresh(imgGliaNorm2(:));
            case 'Median'
                level = median(imgGliaNorm2(:));
            case 'Mean'
                level = mean(imgGliaNorm2(:));
        end
            
        if ThreshMode2 == 1
            BinaryImg2 = zeros(sizeXY(1), sizeXY(2), sizeZ);
            for i = 1:sizeZ
                BinaryImg2(:, :, i) = imbinarize(imgGliaNorm2(:, :, i), level * ThreshSet2);
            end
        else
            BinaryImg2 = imbinarize(imgGliaNorm2, level * ThreshSet2);
        end
                        
        if FillGaps2 == 1
            radiusXY = FillGapsRadius2;
            radiusZ = max(1, round(radiusXY * pxXY / pxZ));
            [x, y, z] = meshgrid(-radiusXY:radiusXY, -radiusXY:radiusXY, -radiusZ:radiusZ);
            se = (x.^2 + y.^2) / radiusXY^2 + z.^2 / radiusZ^2 <= 1;
            BinaryImg2 = imclose(BinaryImg2, se);
        end

        CC = bwconncomp(BinaryImg2, 26); % Find connected components in 3D
        stats = regionprops3(CC, 'Volume'); % Measure voxel count for each object
        volumes = stats.Volume * voxscale; % Convert to physical volume
        keepIdx = find(volumes >= noise2.Min & volumes <= noise2.Max);
        L = labelmatrix(CC);
        NoiseIm2 = ismember(L, keepIdx);
    end
    ...
    if SecondGliaChanYN == 1 
        ...
        if SaveOverlapImg == 1
            ...
            % edit this like single image mode so that dimensions are fixed 
    ...
    clear L BinaryImg BinaryImg2 img2D imgGliaNorm imgGliaNorm2;

    fprintf('Finding glia objects \n');
    ConnectedComponents=bwconncomp(NoiseIm,26);
    numObj = numel(ConnectedComponents.PixelIdxList);
    for i = 1:numObj
    ...
    fprintf('Saving thresholded glia channel \n');
    fig = figure('Visible', 'off');
    imagesc(DetectedObjs);
    colormap(cmapCompress);
    daspect([1 1 1]);
    savefile = ([name '_step1_Glia_Thresh.png']);
    exportgraphics(fig, fullfile(SaveFolderPath, savefile), 'Resolution', 300);
    clear DetectedObjs;
    ...
    fprintf('Finding territorial volume \n');
    ...
    for i = 1:numCells
        % OPTIMIZED: Calculate centroid directly from indices, no 3D array needed
    ...
    centdist = pdist2(centum, centum);
    centdist = nonzeros(centdist);
    ...
    CellDataTable.TerritoryVolume = FullCellTerritoryVol(:);
     ...
     % Clean up Workspace
     clear antiborder AllSeparatedObjs cleared ConnectedComponents fv ...
     nucmask obj object objidx objMask Outline OutlineImage plane Microglia NoiseIm x y z;
    ...
    clearvars -except BatchFolder fullScriptPath imgFiles ParametersFile ...
                  ParametersPath SaveBatchPath ScriptFolder k;
    close all hidden;
    memory
    whos
